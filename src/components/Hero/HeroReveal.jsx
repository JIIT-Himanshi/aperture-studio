  ...
  return (
    <section className='min-h-screen w-full flex flex-col items-center justify-start bg-(--bg)'>
      ...
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, ease: easing }} className='fixed inset-x-0 top-4 md:top-2 z-50 flex justify-center pointer-events-auto'>
        <div className='w-full max-w-368 px-4 md:px-8 lg:px-10'>
      ...
      <div className='mx-auto flex w-full max-w-360 flex-col items-center justify-center gap-8 px-4 pt-20 pb-12 sm:px-6 md:gap-6 md:pt-16 lg:pt-20'>
        ...
        <div className='h-10 md:h-10' aria-hidden='true' />
        ...
        <motion.div initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 1.2, ease: easing, delay: 0.22 }} className='relative flex items-center justify-center'>
          <div className='radial-glow' />
          <motion.img src={cameraSrc} alt='Hero camera' className='cinematic-camera max-h-[48vh] w-auto' style={{ zIndex: 2 }} animate={{ y: [0, -6, 0] }} transition={{ duration: 6, repeat: Infinity, ease: 'easeInOut' }} />
        </motion.div>
